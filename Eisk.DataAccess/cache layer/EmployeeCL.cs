namespace Eisk.CacheLayer
{
    ...
    public sealed class EmployeeCL
    {
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETEMPLOYEE_BY_EMPLOYEEID" + "-", employeeId);
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETEMPLOYEES_BY_REPORTSTO" + "-", reportsTo.ToString(System.Globalization.CultureInfo.CurrentCulture.NumberFormat));
            // See if the item is in the cache
            List<Employee> Employees = GetCacheItem(rawKey) as List<Employee>;
            if (Employees == null)
            {
                // Item not found in cache - retrieve it and insert it into the cache
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETEMPLOYEES_BY_REPORTSTO_PAGED" + "-", reportsTo + "-" + orderBy + "-" + startRowIndex + "-" + maximumRows);
            // See if the item is in the cache
            List<Employee> Employees = GetCacheItem(rawKey) as List<Employee>;
            if (Employees == null)
            {
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETEMPLOYEES_BY_REPORTSTO_PAGED_COUNT" + "-", reportsTo + "-" + orderBy + "-" + startRowIndex + "-" + maximumRows);
            // See if the item is in the cache
        ...
        {
            const string rawKey = "CACHE_EMPLOYEES_GETALLEMPLOYEES";
            // See if the item is in the cache
            List<Employee> Employees = GetCacheItem(rawKey) as List<Employee>;
            if (Employees == null)
            {
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETALLEMPLOYEES_PAGED" + "-", orderBy + "-" + startRowIndex + "-" + maximumRows);
            // See if the item is in the cache
            List<Employee> Employees = GetCacheItem(rawKey) as List<Employee>;
            if (Employees == null)
            {
                // Item not found in cache - retrieve it and insert it into the cache
                Employees = Employee.GetAllEmployeesPaged(orderBy, startRowIndex, maximumRows);
                AddCacheItem(rawKey, Employees);
            }
        ...
        {
            string rawKey = string.Concat("CACHE_EMPLOYEES_GETALLEMPLOYEES_PAGED_COUNT" + "-", orderBy + "-" + startRowIndex + "-" + maximumRows);
            // See if the item is in the cache
            int pagedCount = Convert.ToInt32(GetCacheItem(rawKey), System.Globalization.CultureInfo.CurrentCulture.NumberFormat);
            if (pagedCount == 0)
            {
                // Item not found in cache - retrieve it and insert it into the cache
                pagedCount = Employee.GetAllEmployeesPagedCount(orderBy, startRowIndex, maximumRows);
                AddCacheItem(rawKey, pagedCount);
            }
